<template>
  <div class="app-container">
    ...
    <!--列表-->
    <el-table :data="ajbhouses" highlightCurrentRow @selection-change="selsChange" height="600" border style="width: 100%;">
      ...
      <el-table-column prop="houseCode" label="house_code" width="180" sortable></el-table-column>
      <el-table-column prop="houseName" label="house_name" width="180" sortable></el-table-column>
      <el-table-column prop="floor" label="floor" width="80" sortable></el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="editFormVisible">
      <el-form :model="editForm" labelWidth="80px" ref="editForm">
        ...
        <el-form-item label="单元ID">
          <el-input v-model="editForm.unitId" style="font-size: 15px" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="房屋编码">
          <el-input v-model="editForm.houseCode" style="font-size: 15px"></el-input>
        </el-form-item>
        <el-form-item label="房屋名称">
          <el-input v-model="editForm.houseName" style="font-size: 15px"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import { getAJBHouseListByJKUnitId, getAJBHouseByJKHouseId} from '@/api/ajb'
  export default {
    data: function () {
      return {
        filters: {},
    ...
    methods: {
      ...
      searchHouse: function () {
        ...
        const params = {
          ...
          jkHouseId: that.JKHouse
        };
        if (params.projectId === "" || params.jkBuildId === "" || params.jkUnitId === "") {
          this.$message({message: "请输入小区名称、楼栋名称和单元名称", type: 'warning', center: true});
        } else if (params.jkHouseId === "") {
          //查询该金科单元下映射的安居宝房屋数据
          getAJBHouseListByJKUnitId(params).then(data => {
            console.log(params);
            const {status, msg, result} = data;
            if (status === "success" && result !== null) {
              that.ajbhouses = result;
              console.log(result);
            } else {
          ...
          //根据金科houseId查询映射的安居宝房屋数据
          getAJBHouseByJKHouseId(params).then(data => {
            console.log(params);
            const {status, msg, result} = data;
            if (status === "success" && result !== null) {
              that.ajbhouses = result;
